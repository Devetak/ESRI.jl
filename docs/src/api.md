...
`IndustryInfo(industry_of_firm)` infers the industry count from the largest one-based id and classifies every supplier-customer industry pair as non-essential (`1`), giving a purely linear production function.
...
`ihs_input_classification()` returns the bundled 616 by 616 IHS matrix and `ihs_industry_codes()` returns its label order. The bundled classification is opt-in; `IndustryInfo(industry_of_firm)` uses the purely linear baseline. The bundled data are CC BY 4.0; attribution is in `data/LICENSE.md` in the repository.
