...
const _IHS_DIVISION_GROUPS = Dict(
    lpad(string(division), 2, '0') => group for (group, divisions) in (
        "A01" => 1:1,
        "A02" => 2:2,
        "A03" => 3:3,
        "B" => 5:9,
...
for customer in eachindex(codes), supplier in eachindex(codes)
    ...
    row == 0 ||
        column == 0 ||
        (expected[supplier, customer] = round(UInt8, 2 * source[row, column]))
end

bytes2hex(sha256(join(codes, "\n") * "\n")) == _BUNDLED_LABELS_SHA256 ||
    error("unexpected bundled labels")
bytes2hex(sha256(vec(expected))) == _BUNDLED_CLASSIFICATION_SHA256 ||
    error("unexpected reconstructed matrix")
expected == ihs_input_classification() ||
    error("bundled matrix differs from Zenodo reconstruction")
