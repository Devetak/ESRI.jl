...
function _mode_max_degree(n::Int, mode::Symbol)
    if mode === :truncated_tail
        return _default_max_degree(n)
    elseif mode === :heavy_tail
        return max(n - 1, 1)
    end
    throw(ArgumentError("mode must be :truncated_tail or :heavy_tail"))
end
...
function generate_benchmark_input(
    ...
    alpha::Float64 = 2.3,
    nindustries::Int = 50,
    mode::Symbol = :truncated_tail,
    max_degree::Int = _mode_max_degree(n, mode),
)
    rng = MersenneTwister(seed)
    W, deg = sparse_powerlaw_network(rng, n; mean_degree = mean_degree, alpha = alpha, max_degree = max_degree)
    used_nindustries = min(nindustries, n)
    industry_of_firm = if n <= used_nindustries
    ...
    info = IndustryInfo(industry_of_firm, rand(rng, Bool, used_nindustries))
    return W, info, deg
end
